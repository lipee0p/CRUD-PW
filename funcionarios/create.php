...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nome) || empty($funcao) || empty($data_admissao) || empty($data_nascimento)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO funcionarios (nome, funcao, data_admissao, data_nascimento, salario) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$nome, $funcao, $data_admissao, $data_nascimento, $salario]);
            
            header('Location: read.php?status=created');
            exit();
        } catch (PDOException $e) {
            $error_message = 'Erro ao salvar funcionário: ' . $e->getMessage();
        }
    }
}
...
<div class="mb-8">
    ...
    <p class="text-neutral-400 mt-1 text-sm">Insira os dados do novo colaborador da barbearia.</p>
</div>
...
<div class="max-w-2xl mx-auto glass-card rounded-2xl p-6 sm:p-8">
    <form action="create.php" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" id="nome" name="nome" required placeholder="Ex: Lucas Ferreira" class="w-full bg-neutral-900 border border-neutral-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition-all placeholder:text-neutral-600">
        </div>

        <!-- Função/Cargo -->
        <div>
            <label for="funcao" class="block text-sm font-bold text-neutral-300 uppercase tracking-wider mb-2">Função / Cargo *</label>
            <input type="text" id="funcao" name="funcao" required placeholder="Ex: Barbeiro Principal, Auxiliar, Recepcionista" class="w-full bg-neutral-900 border border-neutral-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition-all placeholder:text-neutral-600">
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="date" id="data_nascimento" name="data_nascimento" required class="w-full bg-neutral-900 border border-neutral-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition-all">
            </div>
            ...
            <div>
                ...
                <input type="date" id="data_admissao" name="data_admissao" required class="w-full bg-neutral-900 border border-neutral-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition-all">
            </div>
        </div>
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="number" step="0.01" min="0" id="salario" name="salario" required placeholder="0,00" class="w-full bg-neutral-900 border border-neutral-800 text-white rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition-all placeholder:text-neutral-600">
            </div>
        </div>
